...
// NEW: AgentsIcon (Slider Animation)
const AgentsIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg width="20" height="20" fill="none" viewBox="0 0 24 24" {...props}>
    <style>
      {`
      .sliders { animation: sliders 1.5s cubic-bezier(0.86, 0, 0.07, 1) infinite alternate both; }
      @keyframes sliders { 0% { transform: translateX(0px); } 100% { transform: translateX(10px); } }
      .sliders-2 { animation: sliders-2 1.5s cubic-bezier(0.86, 0, 0.07, 1) infinite alternate both; }
      @keyframes sliders-2 { 0% { transform: translateX(0px); } 100% { transform: translateX(-10px); } }
      `}
    </style>
    <path stroke="currentColor" strokeLinecap="round" strokeWidth="1.5" d="M6 8.746h12M6 15.317h12"/>
    <circle className="sliders" cx="7.5" cy="8.746" r="1.5" fill="currentColor" stroke="currentColor" strokeWidth="1.5"/>
    <circle className="sliders-2" cx="16.5" cy="15.254" r="1.5" fill="currentColor" stroke="currentColor" strokeWidth="1.5"/>
  </svg>
);


const agentsList = [
  { id: '/general_utility', name: 'General Utility', command: '/general_utility', icon: AgentsIcon },
  { id: '/hr_database', name: 'HR Database', command: '/hr_database', icon: AgentsIcon },
  { id: '/email_security', name: 'Email Security', command: '/email_security', icon: AgentsIcon },
  { id: '/finance_audit', name: 'Finance Audit', command: '/finance_audit', icon: AgentsIcon },
];
...
export const PromptBox = React.forwardRef<HTMLTextAreaElement, React.TextareaHTMLAttributes<HTMLTextAreaElement>>(
  ({ className, ...props }, ref) => {
    ...
    return (
      <div className={cn("flex flex-col rounded-[28px] !px-6 !pt-3 !pb-3 shadow-5xl transition-all duration-300 bg-white/10 backdrop-blur-1xl border border-white/20 cursor-text hover:bg-white/15", className)}>
        ...
        <textarea 
          ...
          placeholder="Message..." 
          className="custom-scrollbar w-full resize-none border-0 bg-transparent !px-2 !pt-4 !pb-2 text-foreground dark:text-white placeholder:text-muted-foreground dark:placeholder:text-gray-300 focus:ring-0 focus-visible:outline-none min-h-12" 
          {...props} 
        />
        
        <div className="mt-0.5 !px-1 pb-2 pt-0">
          <TooltipProvider delayDuration={100}>
            <div className="flex w-full items-center justify-between">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <PopoverTrigger asChild>
                        <button type="button" className="flex h-10 items-center gap-2 rounded-full px-4 py-2 text-sm text-foreground dark:text-white transition-colors hover:bg-accent dark:hover:bg-[#515151] focus-visible:outline-none focus-visible:ring-ring border border-white/5">
                          <AgentsIcon className="h-5 w-5" />
                          <span className="font-medium">Agents</span>
                        </button>
                      </PopoverTrigger>
                    </TooltipTrigger>
                    <TooltipContent side="top" showArrow={true}><p>Select Security Agent</p></TooltipContent>
                  </Tooltip>
                    ...
                    <div className="flex flex-col gap-0.5">
                      {agentsList.map(agent => (
                        <button
                          key={agent.id}
                          onClick={() => {
                            setValue(prev => agent.command + " " + prev.replace(/^\/\w+\s*/, ""));
                            setIsPopoverOpen(false);
                            internalTextareaRef.current?.focus();
                          }}
                          className="flex w-full items-center gap-2 rounded-lg py-2.5 px-4 text-left text-sm hover:bg-accent dark:hover:bg-[#515151] transition-colors group"
                        >
                          <div className="flex flex-col">
                            <span className="font-semibold text-[14px]">{agent.name}</span>
                            <span className="text-[10px] opacity-60 font-medium uppercase tracking-widest">{agent.command}</span>
                          </div>
                        </button>
                      ))}
                    </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button type="submit" disabled={!hasValue} className="flex h-10 w-10 items-center justify-center rounded-full text-sm font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none bg-white/20 text-white hover:bg-white/30 border border-white/20 disabled:bg-white/5">
